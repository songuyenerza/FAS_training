...
def generate_main(model_path, root_dir, all_live_data, batchsize=32):
    ...
    net = torch.nn.DataParallel(net).cuda()
    net.eval()
    print(f"---load model successfully--- {len(train_loader)}")
        ...
        with tqdm.tqdm(total=len(train_loader), desc='Iterate over data') as pbar:
            for img_paths, data in train_loader:
                ...
                for i, mask in enumerate(masks):
                    ...
                    origin_image = cv2.imread(os.path.join(root_dir,  img_path))
                    img_shape = origin_image.shape
                    protocal = img_path.split("/")[0]
                    mask_store_path = os.path.join(root_dir, "train_live_mask", img_name + '.pkl')
...
if __name__ == "__main__":
    ...
    lines = open(os.path.join(root_dir, f'train.txt'), "r").readlines()
    for line in lines:
    ...
    mask_save_path = os.path.join(root_dir, "train_live_mask")
    if not os.path.exists(mask_save_path):
        os.makedirs(mask_save_path)

    print("all_live_data: ", all_live_data)
