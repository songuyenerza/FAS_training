...
def digital_augment(srcFace, mask_path, loc='all', mode='minor'):
    ...
    cv2.imwrite("hullMask.jpg", hullMask*255)
...
def transform_params(img, mode='minor'):
    ...
    if(mode=='minor'):
        color_transform=A.Compose([
            ChannelShuffle(p=1),
            HueSaturationValue(hue_shift_limit=5, sat_shift_limit=5, val_shift_limit=10, always_apply=False, p=1),
            RGBShift(r_shift_limit=10, g_shift_limit=10, b_shift_limit=10, always_apply=False, p=1),
            RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, brightness_by_max=True, always_apply=False, p=1),
            Downscale(scale_min=0.35, scale_max=0.65, interpolation=0, always_apply=False, p=1),
            Sharpen(alpha=(0.1, 0.3), lightness=(0.5, 1.0), always_apply=False, p=1),
            ])
    else:
        color_transform=A.Compose([
...
def convex_hull(mask_path, loc):
    ...
    if(loc=='all'):
        ...
        # where_res = np.where(mask<1) # 1-6 10-13
...
if __name__ == '__main__':
    image = cv2.imread('/data/datasets/thainq/sonnt373/dev/FAS/dev/cvpr2024-face-anti-spoofing-challenge/data_preprocess/face_parsing/datatest/image_T1.jpg')
    mask_path = "/data/datasets/thainq/sonnt373/dev/FAS/dev/cvpr2024-face-anti-spoofing-challenge/data_preprocess/face_parsing/datatest/train_live_mask/image_T1.pkl"
    bld_img = digital_augment(image, mask_path, loc='all', mode='aaa')
    cv2.imwrite("bld_img.jpg", bld_img)
